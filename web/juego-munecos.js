/* ==================================================================
   JUEGO · los muñecos de píxeles para web/juego.html
   ------------------------------------------------------------------
   Es el mismo Compi del vídeo y de los carteles, montado para el
   navegador. Se genera a mano juntando tres trozos, sin tocarlos:

     1. carteles/compi.js hasta POSES.fuga (paleta, piezas y poses)
     2. el bloque `extra` de carteles/peli-sprites.js (caras del habla,
        pelo, barba y gafas de los once, y dibuja())
     3. BEIS, PERSONAS y paleta() de peli-sprites.js
   ...
   sueltos en window: fuera solo sale window.MUNECOS.
   ================================================================== */
window.MUNECOS = (function(){
...
/* en el aire: rodillas arriba y brazos abiertos hacia atrás. La cara
   es la de correr, apretada, porque saltar un obstáculo es esfuerzo y
   con la `feliz` parecía que brincaba de contento */
POSES.salta = function(){
  return { fase:0, dy:-1, lean:1, cara:"corre",
    bI:{ang:218,len:5}, bD:{ang:322,len:5},
    pI:{dx:-1.5,dy:3,kx:-2.5,ky:1.2}, pD:{dx:2.5,dy:2.5,kx:3,ky:0.8} };
};
/* cayendo por el pozo: brazos arriba del todo y piernas colgando */
POSES.cae = function(tt){
  const f = Math.floor(tt/120) % 2;
  return { fase:f, dy:0, cara:"asustado",
    bI:{ang: f?244:236,len:6}, bD:{ang: f?296:304,len:6},
...
/* Cada fotograma se pinta una vez a 27x36 en un canvas propio y luego
   ...
   drawImage, que en un móvil flojo es la diferencia entre 60 y 20 fps. */
/* los que no son de la lista: van de gris y azul apagado para que no
   se confundan con ninguno de los doce, que llevan la camiseta clara */
const OBSTACULOS = {
...
const cache = new Map();
function fotograma(quien, pose, tt, izq){
  const clave = quien + "|" + pose + "|" + tt + (izq ? "|i" : "");
  let lienzo = cache.get(clave);
  if(lienzo) return lienzo;
  const p = quien === "compi" ? null : (PERSONAS[quien] || OBSTACULOS[quien]);
  const T = paleta(p);
  const d = dibuja(pose, tt, { persona: p, polvo: true });
  lienzo = document.createElement("canvas");
  lienzo.width = REJ_W; lienzo.height = REJ_H;
  const c = lienzo.getContext("2d");
  for(let y=0;y<REJ_H;y++){
    let x = 0;
    while(x < REJ_W){
      const k = d.B[y*REJ_W+x];
      if(!k){ x++; continue; }
      let n = 1;
      while(x+n < REJ_W && d.B[y*REJ_W+x+n] === k) n++;
      c.fillStyle = T[k]; c.fillRect(izq ? REJ_W - x - n : x, y, n, 1);
      x += n;
    }
  }
  cache.set(clave, lienzo);
  return lienzo;
}

return { fotograma, TINTA, REJ_W, REJ_H, PISO, QUIETO };
})();
